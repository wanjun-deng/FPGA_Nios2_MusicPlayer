...
void player_init(PlayerState *ps) {
    ...
    IOWR_8DIRECT(FREQSYNTH_IP_0_BASE, 2, ps->volume); // volume_reg @ addr2
}
...
void player_process(PlayerState *ps, const unsigned char *score_data) {
    ...
    switch (ps->status) {
        case STATE_PLAYING:
            ...
            IOWR_8DIRECT(FREQSYNTH_IP_0_BASE, 1, inote); // note_reg
            IOWR_8DIRECT(FREQSYNTH_IP_0_BASE, 0, 1);     // playing_reg = 1
            break;

        case STATE_PAUSED:
            IOWR_8DIRECT(FREQSYNTH_IP_0_BASE, 1, 0);
            IOWR_8DIRECT(FREQSYNTH_IP_0_BASE, 0, 0);
            break;
...
void player_apply_command(PlayerState *ps, alt_u8 cmd) {
    switch (cmd & 0x1F) {
        ...
        case 0x1A: // 音量加
            ...
            IOWR_8DIRECT(FREQSYNTH_IP_0_BASE, 2, ps->volume);
            break;
        case 0x1B: // 音量减
